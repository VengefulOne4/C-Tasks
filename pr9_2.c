...
double** NacitajSpodTrojuholnik_do_dyn_pola(int dyn_r, int dyn_s, double** A)
{
	...
	if (dyn_r < dyn_s)
	{
		for (int i = 1; i < dyn_r; i++)
		{
			for (int j = 0; j < i; j++)
				spod_truholn_dyn_pole[i][j] = A[i][j];
		}
	}
	else
	{
		for (int i = 1; i < dyn_s; i++)
		{
			for (int j = 0; j < i; j++)
				spod_truholn_dyn_pole[i][j] = A[i][j];
		}

		for (int i = dyn_r - dyn_s; i < dyn_r; i++)
			for (int j = 0; j < dyn_s; j++)
				spod_truholn_dyn_pole[i][j] = A[i][j];
...
{
	if (dyn_rA != dyn_rB || dyn_sA != dyn_sB)
	{
		printf("Scitat matice nie je mozne!");
		exit(1);
	}
	else
	{
...
int main()
{
	...
	int rA, sA, rB, sB, navrat_hodn, index;
